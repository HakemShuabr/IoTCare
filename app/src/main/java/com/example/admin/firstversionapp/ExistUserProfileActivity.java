...
public class ExistUserProfileActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_exist_user_profile);
        ...
        findViewById(R.id.deactivate).setOnClickListener(buttonsClickListener);
        ...
        public void onClick(View view) {
            switch (view.getId()){
                case R.id.back:
                    ...
                    homeActivity.putExtra("active", "1");
                                ...
                                for (int i = 0; i < 2; i++) {
                                    ...
                                    for (DataSnapshot child : children) {
                                        if (child.getValue(User.class).getName().toString().equals(username.getText().toString())) {
                                            if(i==0)
                                                userAccount.child("users").child("doctors").child(username.getText().toString()).child("active").setValue(0);
                                            else if(i==1)
                                                userAccount.child("users").child("guardians").child(username.getText().toString()).child("active").setValue(0);
                                            Toast.makeText(ExistUserProfileActivity.this, "user account is deleted", Toast.LENGTH_LONG).show();
                                        }
                                    }
                                }
                            }
                                ...
                                for (int i = 0; i < 2; i++) {
                                    ...
                                    for (DataSnapshot child : children) {
                                        if (child.getValue(User.class).getName().toString().equals(username.getText().toString())) {
                                            ...
                                            Toast.makeText(ExistUserProfileActivity.this, "user account is inactive", Toast.LENGTH_LONG).show();
                                        }
                                    }
                                }
                            }
